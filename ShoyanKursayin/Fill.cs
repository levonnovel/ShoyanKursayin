...
namespace ShoyanKursayin
{
	class Fill
	{
		...
		string AlsoAnswered { get; set; } = "Я либо не знаю ответ на этот вопрос, либо уже отвечал на него!";
		string LastQuestion { get; set; }
		string StandardLongQuestion { get; set; } = " Хотите узнать по подробнее?";
		...
		public string GetAnswer(string currentQuestion)
		{
			...
			if (currentQuestion.ToLower() == "спасибо" || currentQuestion.ToLower() == "круто" || currentQuestion.ToLower() == "здорово")
			{
				LastQuestion = StandardQuestion;
				return LastQuestion;
			}
